...
interface LoginPortalProps {
  onLoginSuccess: (role: "student" | "teacher" | "director", user: any, data: any) => void;
}
...
export default function LoginPortal({ onLoginSuccess }: LoginPortalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || "فشل تسجيل الدخول. يرجى التحقق من المدخلات.");
      }

      onLoginSuccess(result.role, result.user, result.schoolData);
    } catch (err: any) {
